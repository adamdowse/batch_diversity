...
def main():
    ...
    #Setup
    wandb.init(project='Path_Seperation',config=config)

    #Data Generator
    train_DG = DataGens.LocalSUBMODGRADDataGenV2(config['ds_name'],config['batch_size'],config['train_percent'],config['ds_path'],config['alpha'],calc_stats=False)
    test_DG = DataGens.TestDataGen(config['ds_name'], 50, config['test_percent'], config['ds_path'])
    second_train_DG = DataGens.LocalSUBMODGRADDataGenV2(config['ds_name'],128,config['train_percent'],config['ds_path'],config['alpha'],calc_stats=False)
    ...
    while epoch_num <= config['epochs']:
        ...
        if epoch_num % config['n_record_epochs'] == 0 or epoch_num == 2:
            ...
            curvature, curv_var = fim.CurvatureEstimate(model,second_train_DG,config['n_error_scales'],config['n_data_points'])
            #log the curvature
